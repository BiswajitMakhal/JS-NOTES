...
for (let i = 0; i < sec.length; i++) { //prothome i = 0 mane index diye suru holo, then i- ar index gulo jotokhon array-length theke choto thakbe loop cholte thakbe
    if (sec[i] <= 4) {
        console.log(sec[i]);
    }
}
...
let year = 2020;
if (year % 4 === 0 || year % 400 === 0) {  //jei year gulo 4 or 400 die divide kora jai oi gulo leapyear, or jegulo sudhu 100 die divided hoi othoba 4 or 400 die hoina segulo leapyear noi//
...
//  Sum from 1 to 10, loop korte hobe

let sum = 0;
for (i = 1; i <= 10; i++) { //(i = 1; i < 11; i++) arokomo lekha jeto//
    sum += i;
}
console.log(sum);
